...
// stop. Default for timeout could be 60 * time.Second, or as you need.
// The timeout should be longer than the longest upload or download.
func NewCustomHttpServer(addr string, handler http.Handler, timeout time.Duration) *CustomHttpServer {
	ch := make(chan struct{})
	s := &CustomHttpServer{
		Server: http.Server{
			Addr: addr,
			...
			MaxHeaderBytes: 4096,
			ReadTimeout:    timeout, // These are absolute times which must be
			WriteTimeout:   timeout, // longer than the longest {up,down}load.
		},
		ch:    ch,
